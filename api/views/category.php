<ion-view title="<?php echo $extra->name ?>" cache-view="true">
    <ion-content padding="false" ng-controller="WoocommerceCategoryCtrl" class="<?php echo $extra->content_classes." woocommerce category ".$extra->layout;?> "
                 ng-init="baseShowUrl='<?php echo site_url();?>/?appeto_api=woocommerce'; category='<?php echo $args2["filter[category]"]?>'; ck='<?php echo $ck;?>'; cs='<?php echo $cs;?>'"
        >
        <?php
                    ...
                    <div class="col col-50" ng-repeat="product in baseProduct.products">
                        <a href="#/nav/woocommerce/{{ product.id }}/{}/products-get/<?php echo $ck;?>/<?php echo $cs;?>/product/{'name':'{{  product.title }}', 'layout':'<?php echo $extra->layout ?>', 'content_classes':'<?php echo $extra->content_classes ?>', 'slug':'<?php echo $extra->slug ?>'}" style="text-decoration: none">
							...
							<span style="color: #222222 !important; text-align: justify">
								...
								<span ng-if="product.regular_price!=product.price">/</span> <span ng-if="product.regular_price!=product.price" style="text-decoration: line-through">{{ product.regular_price }} ريال</span>
							</span>
                                </div>
                            </div>
                        </a>
